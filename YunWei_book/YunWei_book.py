...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into YunWei_book (title,stars,conments_Num) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except :
...
if __name__ == "__main__":
    big_list = []
    url = 'https://book.douban.com/subject_search?search_text=%E8%BF%90%E7%BB%B4&cat=1001&start=315'
    time.sleep(1)
