...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into zidonghua_linux_book (title,stars,conments_Num) values (%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except :
...
if __name__ == "__main__":
    big_list = []
    url = 'https://book.douban.com/subject_search?search_text=%E8%87%AA%E5%8A%A8%E5%8C%96linux&cat=1001'
    time.sleep(1)
...
#
# create table zidonghua_linux_book(
